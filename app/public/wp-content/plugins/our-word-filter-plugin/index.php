...
/* 
    ...
    Version: 1.0
    Author: Stefan
    Author URI: kttps://www.udemy.com/user/bradshiff/
*/
...
class OurWordFilterPlugin {
    ...
    function replacementFieldHTML() { ?>
        ...
        <p class="description">Leave blenk to simpely remove the filtered words.</p>
    <?php }

    // Tar bort alla kommatecken och trimmar bort alla mellanslag, 
    // returnerar orden trimmade med utbytta till **** samt all övrigt innehåll.
    ...
    // Skriver ut ett bekräftelse meddelande när man submittar formuläret.
    function handleForm() {
        if (wp_verify_nonce($_POST['ourNonce'], 'savedFilterWords') AND current_user_can('manage_options')) {
    ...
    <div class="wrap">
        <h1>Word Filter</h1>
        <!-- Ett formulär där man skriver in vilka ord man vill filtrera.. -->
        <?php if ($_POST['justsubmitted'] == "true") $this -> handleForm() ?>
        <form method="POST">
            <input type="hidden" name="justsubmitted" value="true">
            <?php wp_nonce_field('savedFilterWords', 'ourNonce') ?>
