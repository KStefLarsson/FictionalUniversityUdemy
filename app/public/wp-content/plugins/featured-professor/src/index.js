...
function EditComponent(props) {
  ...
  function updateTheMeta() {
    const professorForMeta = wp.data
    ...
    wp.data.dispatch("core/editor").editPost({meta: {featuredprofessor: professorForMeta}})
  }
      ...
      <div className="professor-select-container">
        <select onChange={e => props.setAttributes({profId: e.target.value})}>
          ...
          {allProfessors.map(professor => {
            return (
              <option value={professor.id} selected={props.attributes.profId == professor.id}>
